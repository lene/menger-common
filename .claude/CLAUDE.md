...
## IMPORTANT: Codebase Intelligence Instructions for menger-common
...
2. `src/main/scala/menger/common/MengerExceptions.scala` — The walk's anchor — its imports fan out the widest in a repo with no single entry point.
3. `src/main/scala/menger/common/Const.scala` — Directly imported by the anchor above; a core collaborator.
4. `src/main/scala/menger/common/ObjectType.scala` — Not on the resolved import paths — import resolution is incomplete for this repo, so links may be missing.
5. `src/main/scala/menger/common/Patterns.scala` — Not on the resolved import paths — import resolution is incomplete for this repo, so links may be missing.
6. `src/main/scala/menger/common/Color.scala` — Not on the resolved import paths — import resolution is incomplete for this repo, so links may be missing.
... and 6 more steps
### Hotspots (High Churn)

...
# extracted verbatim (same marker-line technique as before: `sbt test | tee` returns
# tee's exit code, not sbt's, so capture sbt's real exit via a marker line that survives
# the tee). Adds failed-test-name extraction to satisfy qa-runner.sh's SUITE-line
# contract (O1: failed tests named in the final summary, not just "something failed").
set -u
